...
#define BOTtoken "6641153087:AAFsYV_gKFVhHXjdYT7m_JBobXI89GCJAnQ"
...
String defineCurrentWaterState(){
  if(currentState){
    return F("\U0001F50B Батарейка працює!");
  }
  else{
    return F("\U0001FAAB Батарейка здохла!");
  }
}
...
void sendCurrentWaterState(String ChatId){
  ...
  if(currentState){
    botMessage += F("\n\U000023F1 Новий тест розпочато!");
  }
  else{
    botMessage += F("\n\U000023F1 Життя батарейки тривало ");
